...
output:
  ...
  highlight: kate  # default, tango, pygments, etc.
...
```{r, eval = FALSE}
# Two sources to obtain clinical measurement
# 1. clinical with {medical_code, entity, acdid} link with additional on {entity, acdid}
# 2. test with {medical_code, entity, data1 - data8}

sick_code_cli <- function(n_cli, def_tbl, pat_tbl, data_name, path = ".") {
  ...
  tst_list <- map_dfr(1:n_tst, function(i) {
    read_sas(file.path(path, paste0("test", i, ".sas7bdat"))) %>%
      inner_join(def_tbl, by = "medical_code") %>%
      inner_join(pat_tbl, by = "id") %>%
      select(id, date, medical_code, entity)
    # if the basic patient is built, we may include t0 as well
  })
...
# for proteinuria, we extract from clinical and test seperately
sick_code_cli(n_cli = 2, def_tbl = pu_dx, pat_tbl = pat_pu, data = 'pat_pu_cli')
sick_code_add(n_add = 2, in_data = pat_pu_cli, data_name = "pat_pu_add")

sick_code_tst(n_tst = 2, def_tbl = pu_lab, pat_tbl = pat_pu, data_name = 'pat_pu_tst')

```

Note that the desired value of the clinical measurement may be in different data cell (data1 - data8) depending on the `entity`, we need to go through the lookup table to determine the value and the unit for the corresponding `entity`.
...
# for extracting t0, we may not need pat_tbl

drug <- function(n_ther, def_tbl, pat_tbl, data_name, path = ".") {
...
Note `dosageid` is often used to along with the lookup file `common_dosage.txt` to work along with qty, quantiunit, duration to determine the treatment duration.
